...
class CameraSubsciber(Node):
    ...
    def optical_flow(self, frame):
        ...
        if self.framecounter == 10 or self.old_gray is None:      
         if self.old_gray is None or  300 > len(self.good_new) > 0: 
            self.old_gray = frame_gray
            self.kp = self.uniform_features(frame)
            self.p0 = np.array([kp.pt for kp in self.kp], dtype=np.float32).reshape(-1, 1, 2)
        ...
        if p1 is not None and len(p1[st == 1]) > 0:
            ...
            # Draw the tracks
            for i, (new, old) in enumerate(zip(self.good_new, good_old)):
                a, b = new.ravel()
                c, d = old.ravel()
